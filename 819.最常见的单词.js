...
var mostCommonWord = function(paragraph, banned) {
    let low = paragraph.toLowerCase().match(/[a-z]+/g)
    let up = {}
    let height = 1
    low.forEach((e)=>{
      if(up[e]){
        let count = up[e]['count']+1
        up[e] = { count:count }
    ...
    function getHeight(height){
      let arr = []
      for(let ele in up){
        if(up[ele]['count'] == height){ arr.push(ele) } }
      return arr
    }
    function ban(arr){
      let newArr = [];
      ...
      if(newArr.length <= 0 && height > 1){
        height = height -1
        return ban(getHeight(height))
      }else{ return newArr }
    }
    return ban(getHeight(height))[0]
};
